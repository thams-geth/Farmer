...
public class ProductListAdapter extends RecyclerView.Adapter<ProductListAdapter.ViewHolder> {
    ...
    public ProductListAdapter(List<Product_small> list){
        this.list=list;
    }
    Context  context;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (list.get(position).getImage().isEmpty()){
            Picasso.with(context).load(R.drawable.farmer_logo).into(holder.iv_image);

        }else {
            Picasso.with(context).load(list.get(position).getImage()).placeholder(R.drawable.farmer_logo).into(holder.iv_image);
